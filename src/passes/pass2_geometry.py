...
def detect_overlap_identity_contradictions(
    ...
    for frag_idx, fragment in enumerate(fragments):
        identity_boundary = fragment.get("identity_boundary", {}) or {}
        if identity_boundary.get("starts_after_disappearance", False):
            # Preserve causal boundary created by explicit disappearance split.
            continue
    ...
    for split_info in fragments_to_split:
        ...
        original_boundary = original_fragment.get("identity_boundary", {}) or {}
        fragment_before["identity_boundary"] = {
            "starts_after_disappearance": bool(original_boundary.get("starts_after_disappearance", False)),
            "ends_with_disappearance": False,
        }
        ...
        fragment_after["identity_boundary"] = {
            "starts_after_disappearance": False,
            "ends_with_disappearance": bool(original_boundary.get("ends_with_disappearance", False)),
        }
...
def create_ghost_fragments(
    ...
    # Step 3: Precompute explicit disappearance starts (gap boundaries)
    # A disappearance starts at frame (last_seen + 1) when Pass 1 has a gap.
    disappearance_starts_by_track: dict[str, set[int]] = {}
    for track_id, track in pass1_tracks.items():
        track_frames = sorted(track.get("frames", []))
        starts: set[int] = set()
        for i in range(len(track_frames) - 1):
            if track_frames[i + 1] > track_frames[i] + 1:
                starts.add(int(track_frames[i] + 1))
        disappearance_starts_by_track[str(track_id)] = starts

    # Step 4: For each frame, check if we need ghosts
    ...
    for frame in range(1, max_frame + 1):
        ...
        for track_id in list(active_ghosts.keys()):
            if track_id in present_players:
                ...
                # Smooth ghost trajectory to reappearance boundary.
                track_data_reappear = pass1_tracks.get(track_id, {})
                reappear_frames = track_data_reappear.get("frames", [])
                reappear_centroids = track_data_reappear.get("centroids", [])
                reappear_bboxes = track_data_reappear.get("bboxes", [])
                if (
                    frame in reappear_frames
                    and ghost.get("pixel_centroids")
                    and ghost.get("pixel_bboxes")
                    and reappear_centroids
                    and reappear_bboxes
                ):
                    re_idx = reappear_frames.index(frame)
                    target_centroid = reappear_centroids[re_idx]
                    target_bbox = reappear_bboxes[re_idx]

                    n_pts = len(ghost["pixel_centroids"])
                    if n_pts > 0:
                        last_centroid = ghost["pixel_centroids"][-1]
                        centroid_dx = float(target_centroid[0]) - float(last_centroid[0])
                        centroid_dy = float(target_centroid[1]) - float(last_centroid[1])

                        last_bbox = ghost["pixel_bboxes"][-1]
                        bbox_delta = [float(target_bbox[k]) - float(last_bbox[k]) for k in range(4)]

                        for k in range(n_pts):
                            alpha = (k + 1) / n_pts
                            c = ghost["pixel_centroids"][k]
                            b = ghost["pixel_bboxes"][k]
                            ghost["pixel_centroids"][k] = [
                                float(c[0]) + centroid_dx * alpha,
                                float(c[1]) + centroid_dy * alpha,
                            ]
                            ghost["pixel_bboxes"][k] = [
                                float(b[i]) + bbox_delta[i] * alpha for i in range(4)
                            ]
        ...
        if missing_count > 0:
            ...
            # Create ghosts only at explicit disappearance starts.
            # This anchors ghost lifecycle to occlusion boundaries.
            recently_seen = []
            for track_id in missing_players:
                track_frames = pass1_tracks[track_id].get("frames", [])
                disappearance_starts = disappearance_starts_by_track.get(str(track_id), set())
                if frame not in disappearance_starts:
                    continue
                if any(f < frame for f in track_frames):
                    last_seen = max(f for f in track_frames if f < frame)
            ...
            for track_id, last_seen in recently_seen[:missing_count]:
                if track_id not in active_ghosts:
                    ...
                    if last_seen in track_frames:
                        ...
                        # RULE 1: Lock occluder at occlusion start (same team, nearest)
                        # Find nearest visible player to use as occluder (ONCE, at creation)
                        locked_occluder_track_id = None
                        locked_offset_bbox = None
                        locked_offset_centroid = None
                        min_distance = float('inf')

                        source_team = source_frag.get("team") if source_frag else None
                        ...
                        for other_track_id in present_players:
                            ...
                            # TODO: Team constraint (skip if different team)
                            # For now, just use nearest since Pass 2 doesn't have team assignments yet
                            ...
                            if distance < min_distance:
                                ...
                                locked_occluder_track_id = other_track_id
                                # RULE 2: Compute offset = ghost_last_pos - occluder_pos
                                locked_offset_bbox = [
                                    last_bbox[i] - other_bbox[i] for i in range(4)
                                ]
                                locked_offset_centroid = [
                                    last_centroid[0] - other_centroid[0],
                                    last_centroid[1] - other_centroid[1],
                                ]
                        ...
                        ghost = {
                            ...
                            "mean_confidence": 0.0,
                            "ghost_age": 0,  # Track frames since occlusion started (for smooth interpolation)
                            # RULE 1: Lock occluder at creation
                            "locked_occluder_track_id": locked_occluder_track_id,
                            # RULE 2: Store offset (ghost_pos - occluder_pos)
                            "locked_offset_bbox": locked_offset_bbox,
                            "locked_offset_centroid": locked_offset_centroid,
                            "source_fragment_id": source_frag.get("fragment_id") if source_frag else None,
                        }
                        active_ghosts[track_id] = ghost
                        ghost_counter += 1
                else:
                    # Update existing ghost - RULE 2: follow locked occluder with preserved offset
                    ghost = active_ghosts[track_id]
                    locked_occluder_track_id = ghost.get("locked_occluder_track_id")
                    locked_offset_bbox = ghost.get("locked_offset_bbox")
                    locked_offset_centroid = ghost.get("locked_offset_centroid")

                    if locked_occluder_track_id and locked_occluder_track_id in present_players:
                        # Get current occluder position
                        occluder_track = pass1_tracks.get(locked_occluder_track_id)
                        if occluder_track:
                            occluder_frames = occluder_track.get("frames", [])
                            if frame in occluder_frames:
                                idx = occluder_frames.index(frame)
                                occluder_bbox = occluder_track["bboxes"][idx]
                                occluder_centroid = occluder_track["centroids"][idx]

                                # RULE 2: ghost_pos(t) = occluder_pos(t) + offset
                                if locked_offset_bbox and locked_offset_centroid:
                                    new_bbox = [
                                        occluder_bbox[i] + locked_offset_bbox[i]
                                        for i in range(4)
                                    ]
                                    new_centroid = [
                                        occluder_centroid[0] + locked_offset_centroid[0],
                                        occluder_centroid[1] + locked_offset_centroid[1],
                                    ]

                                    ghost["pixel_bboxes"].append(new_bbox)
                                    ghost["pixel_centroids"].append(new_centroid)
                                else:
                                    # No offset stored - hold position
                                    ghost["pixel_bboxes"].append(ghost["pixel_bboxes"][-1])
                                    ghost["pixel_centroids"].append(ghost["pixel_centroids"][-1])
                            else:
                                # Occluder not visible this frame - hold position
                                ghost["pixel_bboxes"].append(ghost["pixel_bboxes"][-1])
                                ghost["pixel_centroids"].append(ghost["pixel_centroids"][-1])
                        else:
                            # Occluder track not found - hold position
                            ghost["pixel_bboxes"].append(ghost["pixel_bboxes"][-1])
                            ghost["pixel_centroids"].append(ghost["pixel_centroids"][-1])
                    else:
                        # No locked occluder - hold position
                        ghost["pixel_bboxes"].append(ghost["pixel_bboxes"][-1])
                        ghost["pixel_centroids"].append(ghost["pixel_centroids"][-1])
...
def process_clip_pass2(
    ...
    jersey_purity_min_coverage = float(div_cfg.get("jersey_purity_min_coverage_ratio", 0.08))
    if jersey_temporal_cfg.get("enabled", True):
        ...
        while iteration < max_iterations:
            ...
            output_data["fragments"] = detect_jersey_temporal_conflicts(
                ...
                min_coverage_ratio=jersey_purity_min_coverage,
            )
    ...
    for fragment in output_data["fragments"]:
        ...
        frame_count = max(1, int(fragment.get("frame_count", 0)))
        strong_jerseys = []
        for jersey_id, stats in jersey_prob_timeline.items():
            ...
            coverage_ratio = float(count) / float(frame_count)
            if count >= 2 and avg_conf >= jersey_conf_threshold and coverage_ratio >= jersey_purity_min_coverage:
...
def detect_jersey_temporal_conflicts(
    ...
    for frag_idx, fragment in enumerate(fragments):
        identity_boundary = fragment.get("identity_boundary", {}) or {}
        if identity_boundary.get("starts_after_disappearance", False):
            # Respect hard disappearance boundary: downstream jersey logic should
            # not be used to steer identity immediately after reappearance.
            continue
        ...
        for jersey_id, stats in jersey_probs.items():
            ...
            frame_count = max(1, int(fragment.get("frame_count", end_frame - start_frame + 1)))
            coverage_ratio = float(count) / float(frame_count)

            if avg_conf >= appear_threshold and coverage_ratio >= min_coverage_ratio:
        ...
        for i in range(len(timeline)):
            for j in range(i + 1, len(timeline)):
                ...
                if overlap_start <= overlap_end:
                    ...
                    # Respect temporal proximity window for jersey transfer events.
                    first_frame_gap = abs(int(frag_a["jersey_first_frame"]) - int(frag_b["jersey_first_frame"]))
                    if first_frame_gap > temporal_window:
                        continue
    ...
    for split_info in fragments_to_split:
        ...
        # ========================================================================
        # CRITICAL FIX: Prevent infinite loop in jersey temporal exclusivity
        # ========================================================================
        # When splitting at the jersey's FIRST APPEARANCE frame:
        # - fragment_before = frames BEFORE jersey appeared → should NOT have this jersey
        # - fragment_after = frames FROM jersey appearance → SHOULD have this jersey
        #
        # Problem: create_fragment() aggregates ALL jersey_decisions, so BOTH fragments
        # inherit the jersey. This causes the same conflict to be detected repeatedly.
        #
        # Solution: Remove the conflicting jersey from fragment_before's jersey_prob_timeline
        # ========================================================================
        if split_info.get("reason") == "jersey_overlap_conflict":
            jersey_id_to_remove = split_info.get("jersey_id")
            if jersey_id_to_remove is not None:
                # Clear jersey from fragment_before (it appeared AFTER this fragment)
                jersey_timeline_before = fragment_before.get("jersey_prob_timeline", {})
                if jersey_id_to_remove in jersey_timeline_before:
                    del jersey_timeline_before[jersey_id_to_remove]
        ...
        is_jersey_split = split_info.get("reason") in {"jersey_temporal_exclusivity", "jersey_overlap_conflict"}
...
def split_track_into_fragments(
    ...
    gap_split_frame_set = set(gap_split_frames)
    ...
    for split_frame in split_frames:
        ...
        split_is_gap = split_frame in gap_split_frame_set

        effective_split_reason = divergence_for_split
        if split_is_gap and effective_split_reason is None:
            prev_frame_idx = max(0, split_idx - 1)
            frame_gap = int(frames[split_idx] - frames[prev_frame_idx]) if split_idx > 0 else 0
            effective_split_reason = {
                "frame_idx": split_frame,
                "reason": "track_gap_disappearance",
                "metric": float(frame_gap),
                "severity": "hard",
            }

        identity_jump_reason = effective_split_reason.get("reason") if effective_split_reason else None
        ...
        if fragment_length >= min_length_for_fragment:
            ...
            fragment["split_reason"] = effective_split_reason
            fragment["identity_boundary"] = {
                "starts_after_disappearance": starts_after_disappearance,
                "ends_with_disappearance": split_is_gap,
            }
        ...
        start_idx = split_idx
        starts_after_disappearance = split_is_gap
