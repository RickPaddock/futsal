...
source_sha256: sha256:a3d14a9f7caeabe7b10be788b5e43cb90046718b7fb5478836c7df8e05461eea
---
...
Acceptance:
- A CLI command accepts a local video path and emits a deterministic frame/timestamp sequence (frame_index + t_ms) using an explicit timebase rule (including rounding and fps source).
- Failure modes are explicit and actionable (missing file, unsupported codec, decode failure).
- Decoder strategy is explicit and enforced (chosen backend + prerequisite); if unavailable, the command fails with actionable guidance.
...
- The runner is trust-first: when detections are unavailable or uncertain, it emits explicit missing/unknown state and does not fabricate positions; events may be omitted and MUST NOT be emitted as `event_state=confirmed` unless a later intent explicitly raises that bar.
...
- On at least one deterministic UAT clip, the pipeline produces ball track points with `pos_state=present` on at least 10 frames at confidence >= 0.6 and explicit missing elsewhere (no hallucination).
